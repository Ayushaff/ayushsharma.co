  ...
  const scrollToAbout = () => {
    ...
    if (aboutSection) {
      ...
      setTimeout(() => {
        ...
        const offset = -150; // Adjust this value to position the element higher
        window.scrollTo({ top: elementTop + offset, behavior: "instant" });
      }, 500); // Adjust timeout duration if needed
    }
  };
  const scrollToExperience = () => {
    const experienceSection = document.querySelector("#projects");
          ...
          <Stack direction={"row"} spacing={7}>
            {isLargerThanMD ? (
              <>
                <Button
                  variant="ghost"
                  onClick={() => (scroll ? scrollToHero() : onOpen())}
                >
                  Home
                </Button>
